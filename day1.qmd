...
df <- df |>
  ...
  mutate(boy = gender=="male" & age<15, # <2>
    ...
    elderly_male = gender=="male" & age>64,
    elderly_female = gender=="female" & age>64)
```
...
dfhh <- dfhh |>
  ...
  select(y5_hhid, boy, girl, prime_aged_male, prime_aged_female, elderly_male, elderly_female, head_male) # <3>
```
...
dfhh <- dfhh |>
  ...
  summarize(boy = sum(boy, na.rm = TRUE),
    ...
    elderly_male = sum(elderly_male, na.rm = TRUE),
    elderly_female = sum(elderly_female, na.rm = TRUE),
...
#| code-fold: show
ggplot() +
  geom_density(data = df, aes(x = hh_b04))
```
...
#| evaluate: true
ggplot() +
  geom_density(data = df, aes(x = hh_b04, color = hh_b02)) +
  labs(x = "Age", y = "Density") +
  theme_bw()
```
:::
...
ggplot() +
  geom_density(data = df, 
    aes(x = hh_b04, fill = hh_b02), alpha = 0.5) +
  scale_fill_brewer("Gender", palette = "Set2") +
  labs(x = "Age", y = "Density") +
  theme_bw()
```
...
ggplot() +
  geom_density(data = df, 
    aes(x = hh_b04, fill = hh_b02), alpha = 0.5) + # <1>
  scale_fill_brewer("Gender", palette = "Set2") +
  labs(x = "Age", y = "Density") +
  theme_bw() +
  theme(legend.position = "bottom")
